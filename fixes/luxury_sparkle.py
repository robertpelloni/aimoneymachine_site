...
h2 { font-size: 2.3em !important; border-bottom: 2px solid rgba(212,175,55,0.25) !important; padding-bottom: 0.4em !important; }
...
a, a:link, a:visited, a:active { color: #C9A84C !important; text-decoration: none !important; }
...
.widget-title { color: #D4AF37 !important; }
...
function twentytwentyfive_styles() {
    wp_enqueue_style( 'twentytwentyfive-style', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
}

// AI Money Machine - Genuine Opulence
add_action('wp_enqueue_scripts', function() {
    wp_add_inline_style('twentytwentyfive-style', \'"""
    + luxury_css.replace("\\", "\\\\").replace("'", "\\'")
    + r"""');
}, 99);
...
add_action('wp_head', function() {
    ...
    @keyframes borderGlow { 0%,100%{border-color:rgba(184,134,11,0.15)} 50%{border-color:rgba(212,175,55,0.5)} }
    @keyframes floatUp { 0%{transform:translateY(0) scale(1);opacity:1} 100%{transform:translateY(-60px) scale(0.5);opacity:0} }
    </style>\';
}, 1);

// Sparkle particles in wp_head for global effect
add_action(\'wp_body_open\', function() {
    ...
    // Shimmer overlay
    echo \'<div style="position:absolute;top:0;left:0;right:0;bottom:0;background:linear-gradient(90deg,transparent,rgba(255,215,0,0.1),transparent);animation:shimmerSlide 3s ease-in-out infinite;pointer-events:none;z-index:0"></div>\';
    // Scrolling track
    ...
    for ($i = 0; $i < 5; $i++) {
        ...
        foreach ($words as $w) {
            ...
            echo \'<span style="color:#FFD700;font-size:1.2em;font-weight:800;letter-spacing:0.3em;text-transform:uppercase;padding:0 0.8em;text-shadow:0 0 12px rgba(255,215,0,0.5);display:inline-block;animation:pulseWord 2.5s ease-in-out infinite alternate;animation-delay:\'.($idx*0.2).\'s">\' . $emoji . \' \' . $w . \'</span>\';
            echo \'<span style="color:#B8860B;padding:0 0.3em;font-size:0.5em;display:inline-block;animation:pulseDot 1.5s ease-in-out infinite;animation-delay:\'.($idx*0.15).\'s;text-shadow:0 0 6px rgba(184,134,11,0.6)">&#9670;</span>\';
            $idx++;
        }
    }
    echo \'</div></div>\';
...
print("Done - top banner with emojis + sparkles")
